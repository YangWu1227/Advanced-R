...
The function `tryCatch(expr, ..., finally)` registers exiting handlers, which allows use to override the default error behavior.
...
The nuance here is that after the condition is signaled, control passes to the the context where `tryCatch()` was called. In other words, the code that throws the error and creates the error condition object is ever run:
...
In the code above, `base::stop("This is an error", call. = FALSE)` creates a condition object which is passed to the handler as its single argument, and the control is never return to the code. This is different from `base::withCallingHandler()`, which returns the control to where the condition was signaled and `base::stop("This is an error", call. = FALSE)` is run:
...
* Calling handlers returns control to the context of where the condition object was created and the code continues normally. This makes calling handlers work well with non-error conditions.
